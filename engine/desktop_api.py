...
def select_window(request):
    try: d=json.loads(request.body or '{}'); account_id=int(d["account_id"]); hwnd=int(d["hwnd"])
    except (ValueError, KeyError, TypeError, json.JSONDecodeError): return JsonResponse({"error":"account_id/hwnd 无效"},status=400)
    if not _adapter.get_window(hwnd): return JsonResponse({"error":"窗口不存在"},status=404)
    _selected[account_id]=hwnd
    return JsonResponse({"ok":True,"account_id":account_id,"hwnd":hwnd})

def _capture_b64(hwnd):
    image=_adapter.capture(hwnd)
...
def desktop_snapshot(request, account_id: int):
    ...
    if not hwnd:return JsonResponse({"error":"尚未绑定桌面窗口"},status=404)
    info=_adapter.get_window(hwnd)
    if not info:return JsonResponse({"error":"窗口已不存在"},status=404)
    return JsonResponse({"account_id":account_id,"window":info.__dict__,"image":_capture_b64(hwnd),"runtime":_runtime(account_id).snapshot()})

@csrf_exempt
@require_http_methods(["POST"])
def desktop_step(request, account_id: int):
    if account_id not in _selected:return JsonResponse({"error":"尚未绑定桌面窗口"},status=404)
