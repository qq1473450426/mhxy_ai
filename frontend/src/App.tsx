...
import {
  ...
  Home, Inbox, LayoutDashboard, Menu, Package, Search, Settings, ShieldCheck,
  Users, XCircle, Activity, BarChart3, CircleDot, Plus, MoreHorizontal, RefreshCw,
} from 'lucide-react'
...
const navTop = [
  ...
  { label: '任务管理', icon: ClipboardList, active: true },
  { label: '物品管理', icon: Package },
...
function Kpi({ icon: Icon, title, value, tone, trend }: { icon: any; title: string; value: number; tone: string; trend: string }) {
  return <div className={`kpi ${tone}`}><div className="kpiIcon"><Icon size={18} /></div><div className="kpiBody"><div className="kpiTitle">{title}</div><div className="kpiValue">{value}<span className="unit">个</span></div><div className="kpiTrend">较昨日 <b>{trend}</b></div></div></div>
}
...
function App() {
  ...
  useEffect(() => {
    ...
    ping()
    const timer = window.setInterval(() => { ping(); setRefreshAt(new Date()) }, 2000)
    return () => window.clearInterval(timer)
  }, [])

  const stats = useMemo(() => ({
    total: taskRows.length,
    running: taskRows.filter(t => t.status === '运行中').length,
    review: taskRows.filter(t => t.status === '待审核').length,
    done: taskRows.filter(t => t.status === '已完成').length,
    cancelled: taskRows.filter(t => t.status === '已取消').length,
  }), [])
  ...
  return <div className="appShell">
    ...
    <aside className={mobileNav ? 'sidebar open' : 'sidebar'}>
      <div className="sideInner">
        {navGroups.map((group, gi) => <div className="navSection" key={group.title}><div className="sectionTitle">{group.title}<ChevronDown size={14}/></div>{group.items.map((item, idx) => <button className={item === '任务列表' ? 'sideItem active' : 'sideItem'} key={item}><span className="miniIcon">{idx === 0 ? <FileText size={14}/> : <CircleDot size={10}/>}</span>{item}</button>)}</div>)}
    ...
    <main className="mainContent">
      <section className="filterBar">
        ...
        <label>任务类型<select defaultValue="全部类型"><option>全部类型</option><option>主线任务</option><option>日常任务</option></select></label>
        <label>任务状态<select defaultValue="全部状态"><option>全部状态</option><option>运行中</option><option>待审核</option><option>已完成</option></select></label>
        <label>创建时间<div className="dateRange"><input placeholder="开始日期"/><span>至</span><input placeholder="结束日期"/></div></label>
        <button className="primary"><Search size={15}/>查询</button><button className="ghost" onClick={() => setRefreshAt(new Date())}><RefreshCw size={14}/>重置</button>
      </section>

      <section className="kpiGrid">
        <Kpi icon={ClipboardList} title="任务总数" value={156} tone="blue" trend="+12 ↓" />
        <Kpi icon={Activity} title="进行中" value={68} tone="green" trend="+8 ↓" />
        <Kpi icon={FileCheck2} title="待审核" value={23} tone="amber" trend="-3 ↓" />
        <Kpi icon={ShieldCheck} title="已完成" value={65} tone="purple" trend="+7 ↓" />
        <Kpi icon={XCircle} title="已取消" value={8} tone="red" trend="-2 ↓" />
      </section>

      <section className="panel taskPanel">
        <div className="panelHead"><div><h2>任务列表</h2><span>实时同步 · {refreshAt.toLocaleTimeString()}</span></div><div className="panelActions"><button className="ghost">批量操作 <ChevronDown size={14}/></button><button className="ghost">导出数据</button></div></div>
