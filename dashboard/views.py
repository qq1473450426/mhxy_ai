...
from engine.credential_store import encrypt_password, decrypt_password
...
def _account_data(account):
    ...
    return {
        'id': account.id, 'name': account.name, 'account_name': account.account_name,
        'enabled': account.enabled, 'window_title': account.window_title,
        'state': getattr(worker, 'state', 'STOPPED'), 'task': getattr(worker, 'task', '空闲'),
        'progress': getattr(worker, 'progress', 0), 'message': getattr(worker, 'message', ''),
        'world': getattr(worker, 'world', ''), 'updated': getattr(worker, 'updated', None),
        'has_password': bool(account.password), 'auto_login': account.auto_login,
        'auto_reconnect': account.auto_reconnect, 'auto_daily': account.auto_daily,
        'max_reconnect_attempts': account.max_reconnect_attempts,
        'max_backup_switches': account.max_backup_switches,
        'reconnect_delay_seconds': account.reconnect_delay_seconds,
        'backup_account_ids': list(account.backup_accounts.values_list('id', flat=True)),
    }
...
def overview(request):
    task_rows = GMTask.objects.all()
    counts = {row['status']: row['count'] for row in task_rows.values('status').annotate(count=Count('id'))}
...
def leveling_observe(request):
    ...
    observer = ScreenLevelingObserver(hwnd=account.hwnd, window_title=account.window_title, config=PerceptionConfig(), dry_run=True)
...
def accounts(request):
    ...
    data = _body(request)
    account = Account.objects.create(name=data.get('name', '未命名账号').strip() or '未命名账号', account_name=data.get('account_name', '').strip(), password=encrypt_password(data.get('password', '')), login_mode=data.get('login_mode', 'password'), game_exe=data.get('game_exe', ''), window_title=data.get('window_title', '梦幻西游'), launch_args=data.get('launch_args', ''), auto_login=bool(data.get('auto_login', True)), auto_reconnect=bool(data.get('auto_reconnect', True)), auto_daily=bool(data.get('auto_daily', False)), max_reconnect_attempts=max(0, int(data.get('max_reconnect_attempts', 3))), max_backup_switches=max(0, int(data.get('max_backup_switches', 2))), reconnect_delay_seconds=max(1, int(data.get('reconnect_delay_seconds', 15))))
    Worker.objects.create(account=account); Log.objects.create(account=account, event='ACCOUNT_CREATE', message='账号已添加，密码已加密存储')
    _set_backups(account, data.get('backup_account_ids', [])); return JsonResponse(_account_data(account), status=201)


def _set_backups(account, ids):
    valid = Account.objects.filter(id__in=[int(x) for x in ids if str(x).isdigit()]).exclude(id=account.id)
    account.backup_accounts.set(valid)
...
def account_detail(request, pk):
    ...
    Log.objects.create(account=account, event='ACCOUNT_UPDATE', message='账号配置已更新')
...
class _ManagerReconnectExecutor:
    ...
    def switch_account(self, failed_account_id, backup_account_id):
        failed = Account.objects.get(pk=failed_account_id); backup = Account.objects.get(pk=backup_account_id)
        manager.stop(failed); manager.start(backup)
        return {'success': True}
...
def account_action(request, pk):
    ...
    if action == 'start': manager.start(account)
        ...
        state = type('State', (), {'account_id': account.id, 'attempts': 0, 'backup_switches': 0, 'active_account_id': None, 'failed': False, 'events': []})()
        coordinator = ReconnectCoordinator(_ManagerReconnectExecutor(), ReconnectPolicy(account.max_reconnect_attempts, account.max_backup_switches, account.reconnect_delay_seconds))
        ...
        Log.objects.create(account=account, event='RECONNECT', message=json.dumps(result, ensure_ascii=False))
        return JsonResponse({'account': _account_data(account), 'result': result})
